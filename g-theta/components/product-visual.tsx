...
import { HeroSilhouette } from "@/components/hero-silhouettes"

export function ProductVisual({ product }: { product: Product }) {
  return (
    <div
      className="relative flex aspect-[4/5] w-full flex-col items-center justify-center gap-3 overflow-hidden rounded-[var(--radius)]"
      style={{
    ...
    >
      ...
      <HeroSilhouette id={product.silhouette} className="relative h-20 w-20 text-white/85 drop-shadow-[0_6px_16px_rgba(0,0,0,0.35)] sm:h-24 sm:w-24" />
      <p className="relative px-6 text-center font-display text-xl font-black uppercase leading-tight text-white drop-shadow-[0_4px_12px_rgba(0,0,0,0.35)] sm:text-2xl">
        {product.print}
      </p>
